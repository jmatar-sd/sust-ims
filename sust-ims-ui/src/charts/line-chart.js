var charts_LineChart	= function(){
/**
 * 
 */
	this.onStateChange = function() {
		var lineChart =	new Chartist.Line('.line-chart', 
			  { labels: this.getState().labels, series: this.getState().series } 
			 ,{
			  high: this.getState().high,
			  low:  this.getState().low,
			  showArea: this.getState().showArea,
			  showLine: this.getState().showLine,
			  showPoint:this.getState().showPoint,
			  fullWidth:this.getState().fullWidth,
			  axisX: {
			    showLabel: this.getState().showLabel,
			    showGrid: this.getState().showGrid
			  }
		});
		
		// Let's put a sequence number aside so we can use it in the event callbacks
		if(this.getState().animationEnabled){
		var seq = 0,
		  delays 	= this.getState().animationDelays,
		  durations = this.getState().animationDurations;
